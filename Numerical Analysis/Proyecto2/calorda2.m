function [Z] = calorda2(a,b,tmin,tmax,N,M)
...
for j = 2:M
    ...
    % dimensión N-2
    ld = Z(2:N-1, j-1);              %llenamos con los valores del piso anterior
    ld(1) = ld(1) + sigma*w1(j);     %ld(1) correspondiente al primer valor del vector
    ld(N-2) = ld(N-2) + sigma*w2(j); %ld(N-2) correponde al úlimo valor del vector  
