...
% Out: p ... The dogleg point
    ...
    if alphau >= delta/escala
        p = -delta/escala * g; %Punto de Cauchy
    else
        pb = -linsolve(B,g);    
        if norm(pb) <= delta
            p = pb; %Punto de Newton
        else %Si no es ninguno de los dos entonces se calcula el punto dogleg
            resta = pb - pu;
            u = dot(resta,pu);
            nu = norm(resta)^2;
            ...
            p = pu + alpha*resta; %Combinacion lineal de Cauchy y Newton
        end
    end    
end
